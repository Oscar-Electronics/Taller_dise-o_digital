// axi_ram_adapter.v
// Esclavo AXI4-Lite lectura/escritura para la RAM
...
module axi_ram_adapter #(
    ...
    always @(posedge s_aclk or negedge s_aresetn) begin
        if (!s_aresetn) begin
            ...
            if (s_axi_arvalid && !read_valid) begin
                ...
                read_valid <= 1'b1;
            end else if (s_axi_rready && read_valid)
                read_valid <= 1'b0;
        end
    end
    ...
    always @(posedge s_aclk or negedge s_aresetn) begin
        if (!s_aresetn) begin
            ...
            if (s_axi_awvalid && s_axi_wvalid && !write_ready)
                write_ready <= 1'b1;
            else if (write_ready) begin
                write_ready <= 1'b0;
                write_done  <= 1'b1;
            end else if (write_done && s_axi_bready)
                write_done <= 1'b0;
        end
    end
endmodule